...
│   └── templates/           # Custom HTML templates (created by polis render --init-templates)
│       ├── post.html
│       ├── comment.html
│       ├── comment-inline.html
│       └── index.html
...
**What it does:**
1. Converts all markdown files in `posts/` and `comments/` to HTML
2. Uses pandoc for markdown-to-HTML conversion
3. Applies customizable HTML templates with metadata substitution
4. Embeds blessed comments directly in post HTML files
5. Appends the original markdown source (with frontmatter) as an HTML comment
6. Generates an `index.html` listing all posts
7. Skips files where HTML is newer than markdown (unless `--force`)
...
Create custom templates with `polis render --init-templates`:

```bash
polis render --init-templates
```

This creates `.polis/templates/` with four template files:
- `post.html` - Template for post pages
- `comment.html` - Template for standalone comment pages
- `comment-inline.html` - Template for comments embedded in posts
- `index.html` - Template for the site index
...
| `{{blessed_comments}}` | Rendered blessed comments (posts only) |
...
| `{{posts_list}}` | List of posts (index only) |
| `{{post_count}}` | Number of posts (index only) |
| `{{year}}` | Current year |

Edit these templates to customize the HTML output, then run `polis render` to apply your changes.
